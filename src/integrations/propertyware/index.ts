...
export class PropertyWareIntegration extends EventEmitter {
  ...
  private setupEventForwarding(): void {
    ...
    // Forward sync events
    this.adapter.on('sync:started', (data: any) => {
      this.emit('sync:started', data);
    });
    
    this.adapter.on('sync:progress', (data: any) => {
      this.emit('sync:progress', data);
    });
    
    this.adapter.on('sync:completed', (data: any) => {
      this.emit('sync:completed', data);
    });
    
    this.adapter.on('sync:error', (data: any) => {
  ...
  async getBuildings(portfolioId?: string): Promise<any[]> {
    if (this.isDryRun) {
      ...
      return portfolioId 
        ? buildings.filter((b: any) => b.portfolioKey === portfolioId)
        : buildings;
    }
  ...
  async getAllEntities(entityType: string, options?: any): Promise<any[]> {
    return this.adapter.getAllEntities(entityType);
  }
